...
import { Wallet as WalletIcon, Check, CreditCard, Gift } from 'lucide-react';
...
export const Wallet = () => {
  ...
  const handleRedeemCoupon = async () => {
    if (!couponCode.trim()) {
      toast.error('Add meg a kuponkódot!');
      return;
    }
    
    setRedeemingCoupon(true);
    try {
      const res = await api.post('/wallet/redeem-coupon', { code: couponCode });
      toast.success(res.data.message);
      setCouponCode('');
      fetchWalletData();
      fetchTransactions();
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Érvénytelen kuponkód');
    } finally {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        <Card className="bg-zinc-900/50 border-white/5">
          <CardHeader>
            <h2 className="font-chakra text-2xl font-bold uppercase text-white flex items-center gap-2">
          ...
          <CardContent>
            <div className="flex gap-3">
              ...
              >
                {redeemingCoupon ? 'Beváltás...' : 'Beváltás'}
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ul className="space-y-3 text-zinc-300">
              <li className="flex items-start gap-3">
                ...
                <span>Kiemelt események létrehozása (1000 Ft/hét)</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-primary font-bold">•</span>
                  ...
                  >
                    <div>
                      <p className={`font-semibold ${tx.amount > 0 ? 'text-green-400' : 'text-red-400'}`}>
                        {tx.amount > 0 ? '+' : ''}{tx.amount} Ft
                      </p>
                      <p className="text-xs text-zinc-500 uppercase tracking-wider mt-1">
                        {tx.payment_method === 'manual_add' ? 'Manuális hozzáadás' : 
                         tx.payment_method === 'manual_subtract' ? 'Manuális levonás' : 
                         tx.payment_method}
                      </p>
                    </div>
